...
int main()
{
    ...
    std::string outputPathStr = "all_time_oh_sf_month_averages.csv";
    if (writeAveragesToCSV(yearlyMeans, outputPathStr))
    {
        std::cout << "Averages file written to " << outputPathStr << " .\n";
#ifdef _WIN32
        std::string command = "python solarMonthlyGrapher.py ";
#else
        std::string command = "python3 solarMonthlyGrapher.py ";
#endif
